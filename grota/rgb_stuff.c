...
void rgb_matrix_layer_indicator_custom(void) {
  ...
  RGB background2 =
      hsv_to_rgb((HSV){.h = ((rgb_matrix_config.hsv.h + 40) % 0xFF),
                       .s = rgb_matrix_config.hsv.s,
                       .v = (rgb_matrix_config.hsv.v >> 2)});

  RGB rgb1 = hsv_to_rgb((HSV){.h = (rgb_matrix_config.hsv.h + 80) % 0xFF,
                              .s = rgb_matrix_config.hsv.s,
                              .v = rgb_matrix_config.hsv.v});
  RGB rgb2 = hsv_to_rgb((HSV){.h = (rgb_matrix_config.hsv.h + 120) % 0xFF,
                              .s = rgb_matrix_config.hsv.s,
                              .v = rgb_matrix_config.hsv.v});
  RGB rgb3 = hsv_to_rgb((HSV){.h = (rgb_matrix_config.hsv.h + 160) % 0xFF,
                              .s = rgb_matrix_config.hsv.s,
                              .v = rgb_matrix_config.hsv.v});
