...
void matrix_scan_play_audio_when_mods_are_hold(void) {
  // Bail out if a space cadet key has been pressed but not enough time has passed to trigger a hold.
  if (tapping_term_for_space_cadet_key != 0 && timer_elapsed(timer_sc) < tapping_term_for_space_cadet_key) {
    return;
  }
